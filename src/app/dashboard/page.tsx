import { BadgeDollarSign, UsersRound, WalletCards } from "lucide-react";
...
import type { BudgetRow, ExpenseRow } from "@/types/app";
...
export default async function DashboardPage() {
  ...
  const byMember = expensesByMember(monthExpenses, members);
  ...
  return (
    <AppShell coupleId={couple.id} members={members}>
      <section className="hero-panel rounded-[8px] p-6 md:p-8">
        <div className="relative z-10 grid gap-6 lg:grid-cols-[minmax(0,1.2fr)_minmax(280px,0.8fr)]">
          <div className="space-y-5">
            <span className="surface-chip">Resumo do casal</span>
            <div className="max-w-2xl space-y-3">
              ...
              <p className="max-w-xl text-sm leading-7 text-white/72 md:text-base">
                {topCategory
                  ? `${topCategory} tá pesada em ${monthLabel(start)}.`
                  : "Esse mês tá tranquilo por enquanto."}
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <span className="surface-chip">
                <UsersRound size={14} />
                {byMember.map((item) => `${item.name}: ${formatCurrency(item.value)}`).join(" · ") ||
                  "Sem gastos"}
              </span>
              <span className="surface-chip">
              ...
              <span className="surface-chip">
                <WalletCards size={14} />
                Meta do mês {budget ? formatCurrency(Number(budget.amount)) : "sem meta"}
              </span>
            </div>
          </div>
          ...
          <div className="grid gap-3">
            <div className="rounded-[8px] bg-white/10 p-4 backdrop-blur">
              <p className="text-[11px] font-black uppercase tracking-[0.16em] text-white/60">
                A grana foi pra onde?
              </p>
              <p className="mt-2 text-2xl font-black text-white">{topCategory ?? "Sem categoria"}</p>
              <p className="mt-1 text-sm font-medium text-white/72">
                {topCategory ? "Categoria mais forte do mês" : "Lança o primeiro gasto e tudo começa a aparecer"}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-[8px] bg-white/10 p-4 backdrop-blur">
                ...
                <p className="mt-2 text-xl font-black text-white">{formatCurrency(byMember[0]?.value ?? 0)}</p>
              </div>
              <div className="rounded-[8px] bg-white/10 p-4 backdrop-blur">
                <p className="text-[11px] font-black uppercase tracking-[0.16em] text-white/60">
                  Isabela
                </p>
                <p className="mt-2 text-xl font-black text-white">{formatCurrency(byMember[1]?.value ?? 0)}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        ...
        <SummaryCard
          label="Quem gastou?"
          value={byMember.map((item) => `${item.name}: ${formatCurrency(item.value)}`).join(" · ") || "Sem gastos"}
          icon={<UsersRound size={18} />}
        />
        <SummaryCard
          label="A grana foi pra onde?"
          value={topCategory ?? "Sem categoria"}
          hint={topCategory ? "Categoria campeã do mês" : "Lança o primeiro gasto"}
          tone="purple"
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <section className="grid gap-4">
          {budget ? (
            <BudgetProgress label="Meta mensal" spent={total} budget={Number(budget.amount)} />
          ) : (
