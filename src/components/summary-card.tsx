...
export function SummaryCard({
  ...
  return (
    <article
      className={cn(
        "min-w-0 overflow-hidden rounded-[8px] p-4 shadow-sm ring-1 ring-black/5",
        toneClass,
      )}
    >
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-2 line-clamp-2 break-words text-xl font-black leading-tight tracking-normal text-[#111827] sm:text-2xl">
            {value}
          </p>
        </div>
        {icon ? (
          <span className={cn("self-start rounded-[8px] p-2 shadow-sm ring-1 ring-black/5", iconClass)}>{icon}</span>
        ) : null}
      </div>
